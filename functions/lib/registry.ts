...
export function buildRegistry(env: Env): RegistrySnapshot {
  ...
  const browserRun = statusForSecret(env, ['CLOUDFLARE_BROWSER_RUN_TOKEN'], 'Cloudflare Browser Run credentials are configured server-side; only documented public workflows are eligible.')
  const providers: ProviderDefinition[] = [
    ...
    { id: 'optional.cloudflare.browser_run', label: 'Cloudflare Browser Run', category: 'browser', status: browserRun.status, auth: 'binding', configKeys: ['CLOUDFLARE_BROWSER_RUN_TOKEN'], capabilities: ['web.scrape.browser', 'document.pdf', 'image.extract'], supportedTasks: ['public_browser_observation', 'public_pdf', 'public_screenshot'], requiresBrowser: true, requiresJavaScript: true, costClass: 'usage-based', expectedLatency: 'high', lastVerified: checkedAt, fallbackProviderIds: ['local.evidence', 'manual.evidence'], details: browserRun.details, policy: 'Public permitted workflows only; no CAPTCHA, login, paywall, or anti-bot evasion.' },
  ...
  const capabilities: CapabilityDefinition[] = [
    ...
    { id: 'web.scrape.browser', label: 'Permitted browser observation', providerIds: ['optional.cloudflare.browser_run', 'optional.apify.actors'], defaultProviderId: 'optional.cloudflare.browser_run', taskTypes: ['public_browser_observation', 'public_scrape'], urlTypes: ['public_dynamic'], browserRequired: true, javascriptRequired: true, status: providerStatus('optional.cloudflare.browser_run') === 'ready' || providerStatus('optional.apify.actors') === 'ready' ? 'ready' : 'not_configured', fallbackCapabilityIds: ['web.extract.public_metadata', 'evidence.manual'], details: 'Optional browser-capable public route. It is never an access-control bypass.' },
  ...
  return { checkedAt, providers, capabilities, policy: { automaticRouteSelection: true, externalRequests: 'metadata-only-unless-configured', maxRetryAttempts: 2, maxRedirects: 3, credentials: 'server-only' } }
}
