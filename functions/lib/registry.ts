...
export function buildRegistry(env: Env): RegistrySnapshot {
  ...
  const capabilities: CapabilityDefinition[] = [
    ...
    { id: 'market.ebay_us', label: 'eBay US market research', providerIds: ['official.ebay.browse', 'optional.apify.actors'], defaultProviderId: 'official.ebay.browse', taskTypes: ['market_research', 'listing_comparison'], urlTypes: ['ecommerce_product'], browserRequired: false, javascriptRequired: false, status: providerStatus('official.ebay.browse') === 'ready' || providerStatus('optional.apify.actors') === 'ready' ? 'ready' : 'not_configured', fallbackCapabilityIds: [], details: 'Official Browse API when configured; otherwise the documented Apify eBay actor when runtime-verified; otherwise a truthful not-configured state.' },
