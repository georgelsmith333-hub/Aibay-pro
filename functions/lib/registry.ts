...
export function buildRegistry(env: Env): RegistrySnapshot {
  ...
  const providers: ProviderDefinition[] = [
    ...
    { id: 'local.ebay-scraper', label: 'Built-in eBay search reader', category: 'local', status: 'ready', auth: 'none', configKeys: [], capabilities: ['market.ebay_us'], supportedTasks: ['market_research', 'listing_comparison'], requiresBrowser: false, requiresJavaScript: false, costClass: 'free-local', expectedLatency: 'medium', lastVerified: checkedAt, fallbackProviderIds: ['official.ebay.browse', 'optional.apify.actors', 'manual.evidence'], details: 'Parses public eBay search result pages with bounded requests; visible page data only; respects target controls and reports blocks truthfully. No credentials required.', policy: 'No CAPTCHA/login/paywall bypass; bounded per-domain requests; provenance on every listing.' },
  ]
  ...
  const capabilities: CapabilityDefinition[] = [
    ...
    { id: 'market.ebay_us', label: 'eBay US market research', providerIds: ['local.ebay-scraper', 'official.ebay.browse', 'optional.apify.actors'], defaultProviderId: 'local.ebay-scraper', taskTypes: ['market_research', 'listing_comparison'], urlTypes: ['ecommerce_product'], browserRequired: false, javascriptRequired: false, status: 'ready', fallbackCapabilityIds: [], details: 'Three-tier route: built-in public-page reader (free, no keys) → official Browse API when configured → optional Apify actor when runtime-verified. Blocks and limits are reported truthfully.' },
    ...
    { id: 'supplier.discovery', label: 'Supplier discovery', providerIds: ['local.evidence'], defaultProviderId: 'local.evidence', taskTypes: ['supplier_match'], urlTypes: ['static_product'], browserRequired: false, javascriptRequired: false, status: 'ready', fallbackCapabilityIds: ['evidence.manual'], details: 'Discovers supplier offers from public marketplace pages (zero keys, bounded); matches by title fingerprint; blocks reported truthfully.' },
    { id: 'winning.finder', label: 'Auto winning-item finder', providerIds: ['local.evidence'], defaultProviderId: 'local.evidence', taskTypes: ['winning_finder'], urlTypes: ['search_results'], browserRequired: false, javascriptRequired: false, status: 'ready', fallbackCapabilityIds: ['evidence.manual'], details: 'Combines live eBay search observations with supplier discovery into explainable winning-item candidates with gross-margin estimates.' },
    { id: 'market.sold_data', label: 'Public sold-item observations', providerIds: ['local.evidence'], defaultProviderId: 'local.evidence', taskTypes: ['sold_research'], urlTypes: ['search_results'], browserRequired: false, javascriptRequired: false, status: 'ready', fallbackCapabilityIds: ['evidence.manual'], details: 'Reads eBay public completed/sold search results (zero keys, bounded) into sold-price observations with sample size and date coverage. Not the official sold-data API; always labeled as a public-page sample.' },
  ]
  return {
    checkedAt,
    providers,
