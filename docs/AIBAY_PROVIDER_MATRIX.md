...
| `local.ebay-scraper` (Built-in eBay search reader) | `market.ebay_us` | None — zero keys | Pages runtime; bounded public-page reader (12s timeout, 3 redirects eBay-only, ~3MB cap) | `ready` (always, local; target may block and that is reported truthfully) | `public-search-page-v1` | `official.ebay.browse`, `optional.apify.actors`, `manual.evidence` | VERIFIED MOCKED E2E (9/9: parse, currency, sold, blocks, rate limit, redirect safety, three-tier endpoint); REAL EXTERNAL requires the deployed site |
...
| `apify.ebay-scraper` (eBay via Apify — the no-eBay-dev-app path) | `market.ebay_us` | `APIFY_API_TOKEN`, `APIFY_EBay_ACTOR` (default `dtrungtin/ebay-scraper`), `APIFY_EBay_CANARY` — server-side | Apify documented Actor API (run start → poll → dataset fetch) | `ready` only after a runtime-verified canary (`APIFY_EBay_CANARY`); `not_configured` otherwise | actor-versioned (run/dataset provenance attached to every result) | `local.evidence`, `manual.evidence` | UNCONFIGURED until token + canary; implemented, bounded (90s poll budget, 50-item cap) and tested (6/6 mocked e2e) |
